...
- live GitHub main branch protection readiness audit under `scripts/check-github-main-protection.py`, checking strict required CI status checks, force-push disabling, branch-deletion disabling, and optional PR-review/admin-enforcement policy without printing tokens, logs, payloads, or branch-protection API bodies
- live GitHub Actions permissions readiness audit under `scripts/check-github-actions-permissions.py`, checking selected-action repository admission, read-only default workflow tokens, disabled Actions pull-request approvals, GitHub-owned action allowance, and the required `dtolnay/rust-toolchain@stable` marketplace pattern without printing tokens, logs, payloads, or workflow contents
- local GitHub workflow permissions readiness audit under `scripts/check-github-workflow-permissions.py`, checking explicit read-only workflow defaults, forbidden trigger surfaces, and expected release-only write scopes without printing tokens, secrets, workflow contents, logs, or payloads
- live GitHub repository security readiness audit under `scripts/check-github-repository-security.py`, checking Dependabot vulnerability alerts/security updates, secret scanning, push protection, and open Dependabot/secret-scanning alert counts without printing alert bodies, repository secrets, tokens, logs, or payloads
