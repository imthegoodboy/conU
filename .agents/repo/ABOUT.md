...
- local and admin-gated hosted account suspension through `conu-relay --hosted-account-suspend` and `conu-relay --admin-hosted-account-suspend`, plus guarded local fleet account/node audit and suspension through `conu-relay --hosted-fleet-account-audit` and `conu-relay --hosted-fleet-account-suspend`, reporting account-wide or node-scoped credential/tenant consistency warnings before mutation, revoking tenant metadata before account credential records in account-wide mode, and revoking tenant-node metadata before node credential records with `--node` while reporting metadata-only counts and display guards
