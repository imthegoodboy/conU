...
- Relay mailbox stores encrypted envelopes only; `conu-relay --mailbox-audit` and `conu-relay --admin-mailbox-audit` report retention metadata only and may load metadata-only retention policy files; `conu-relay --mailbox-purge` and `conu-relay --admin-mailbox-purge` report dry-run metadata without deleting and delete expired valid durable files only with explicit confirmation; relay-local scheduled purge deletes expired valid durable files only when explicitly configured; relay accounting and relay abuse files store metadata counters only; abuse threshold reports evaluate those counters only, may load metadata-only threshold policy files, and may fail the process for automation only when `--fail-on-threshold` is set. Distributed hosted mailbox retention orchestration beyond single-relay purge and distributed adaptive abuse workflows remain future work.
