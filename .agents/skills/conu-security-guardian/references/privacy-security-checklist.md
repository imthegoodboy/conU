...
- `conu-relay --mailbox-purge` output may contain durable mailbox scanned counts, invalid counts, expired counts, purged counts, byte totals, optional validated node ids, positive TTL metadata, dry-run/confirmed mode, and display guards only. It must require exactly one of `--dry-run` or `--confirm`; dry-run must delete nothing, and confirmed mode may delete only expired valid `.mailbox` files. It must not print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, plaintext payloads, session ids, private keys, arbitrary frame bodies, message text, stream chunks, or room-event plaintext.
...
- Relay mailbox stores encrypted envelopes only; `conu-relay --mailbox-audit` and `conu-relay --admin-mailbox-audit` report retention metadata only; `conu-relay --mailbox-purge` reports dry-run metadata without deleting and deletes expired valid durable files only with explicit confirmation; relay-local scheduled purge deletes expired valid durable files only when explicitly configured; relay accounting and relay abuse files store metadata counters only. Destructive distributed hosted mailbox retention orchestration and distributed adaptive abuse workflows remain future work.
