...
- `conu-relay --hosted-account-suspend`, `conu-relay --hosted-fleet-account-suspend`, and `conu-relay --admin-hosted-account-suspend` output may contain action/status, account id, credential counts, tenant/node counts, hosted policy counts, credential and tenant paths or relay endpoint, mode, and display guards only. They must revoke the tenant before revoking account credential records, must not weaken local peer policy, and the admin-hosted form must require an admin token read from stdin with full-admin access or both credential and tenant scopes. The fleet form must reject partial credential/tenant source entries and preflight all complete local sources before confirmed mutation. They must not echo the admin token on success or failure, or print raw node tokens, token hashes, private keys, relay session ids, plaintext payloads, ciphertext bodies, arbitrary frame contents, message text, stream chunks, room-event plaintext, or manifest contents.
