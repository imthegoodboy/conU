...
- `conu-relay --mailbox-audit` and `conu-relay --admin-mailbox-audit` output may contain durable mailbox file counts, byte totals, queued timestamp bounds, optional expired counts for an operator-provided TTL, invalid mailbox-file counts, optional validated node ids, and display guards only. The admin-hosted form must require an admin token read from stdin and must not echo that token on success or failure. Neither form may print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, plaintext payloads, session ids, private keys, arbitrary frame bodies, message text, stream chunks, or room-event plaintext.
- `conu-relay --mailbox-purge` and `conu-relay --admin-mailbox-purge` output may contain durable mailbox scanned counts, invalid counts, expired counts, purged counts, byte totals, optional validated node ids, positive TTL metadata, dry-run/confirmed mode, and display guards only. The admin-hosted form must require an admin token read from stdin and must not echo that token on success or failure. Both forms must require exactly one of `--dry-run` or `--confirm`; dry-run must delete nothing, and confirmed mode may delete only expired valid `.mailbox` files. They must not print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, plaintext payloads, session ids, private keys, arbitrary frame bodies, message text, stream chunks, or room-event plaintext.
...
- Relay mailbox stores encrypted envelopes only; `conu-relay --mailbox-audit` and `conu-relay --admin-mailbox-audit` report retention metadata only; `conu-relay --mailbox-purge` and `conu-relay --admin-mailbox-purge` report dry-run metadata without deleting and delete expired valid durable files only with explicit confirmation; relay-local scheduled purge deletes expired valid durable files only when explicitly configured; relay accounting and relay abuse files store metadata counters only. Distributed hosted mailbox retention orchestration beyond single-relay purge and distributed adaptive abuse workflows remain future work.
