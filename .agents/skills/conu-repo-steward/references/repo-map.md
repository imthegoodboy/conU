...
crates/conu-relay
  small plain WebSocket relay service, offline scoped credential issuance, helper-driven manifest upsert/rotate/revoke, account-scoped online admin lifecycle, metadata-only hosted tenant registry, session auth/resume, metadata-only file-backed session state and session-state audit, live-reloaded hashed scoped credential manifests, metadata-only accounting/quotas, blind ciphertext forwarding, relay/bootstrap groundwork
