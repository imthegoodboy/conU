...
Guarded fleet account audit and suspension are local operator tooling only: `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file <path> [--fail-on-warning]` may report credential/tenant consistency warnings, and `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> (--dry-run|--confirm)` may update manifest-listed local credential and tenant files after preflight, but neither command must contact remote relays, inspect payloads, print manifest contents, or claim to be distributed tenant lifecycle automation.
