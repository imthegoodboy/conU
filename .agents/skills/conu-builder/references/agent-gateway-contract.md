...
Guarded fleet account suspension is local operator tooling only: `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> (--dry-run|--confirm)` may update manifest-listed local credential and tenant files after preflight, but it must not contact remote relays, inspect payloads, print manifest contents, or claim to be distributed tenant lifecycle automation.
