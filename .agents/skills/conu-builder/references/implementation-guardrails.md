...
- `conu-relay --hosted-fleet-dashboard` may aggregate a versioned manifest of multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse stores. The manifest must require explicit false display guards; relative paths resolve from the manifest directory; output may include only source paths, relay names, filters, aggregate counters, and false display guards. It must not output raw tokens, admin tokens, token hashes, private keys, session ids, payloads, ciphertext bodies, arbitrary frame contents, message text, stream chunks, room-event plaintext, or manifest contents. It is still an operator snapshot, not hosted billing, adaptive abuse automation, distributed locking, or distributed retention orchestration.
