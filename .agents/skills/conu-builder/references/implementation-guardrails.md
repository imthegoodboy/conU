...
- npm install scripts may download release archives and copy binaries, but must require HTTPS except loopback HTTP smoke servers, reject embedded download URL credentials, enforce explicit download timeout and byte limits, require strict checksum files that name the downloaded archive, hash archives in chunks before extraction, avoid printing URL query strings or fragments in download errors, preflight archive member names and unsupported link entries before extraction, bound extracted-tree scanning before binary selection, require extracted binaries to come from the expected release root `bin/` directory without duplicate binary names elsewhere, and must not read, upload, log, or package local conU state.
