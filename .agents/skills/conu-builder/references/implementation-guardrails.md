...
- `conu-relay --hosted-fleet-dashboard` may aggregate a versioned manifest of multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse stores. The manifest must require explicit false display guards; relative paths resolve from the manifest directory; output may include only source paths, relay names, filters, aggregate counters, optional aggregate abuse threshold check counts/exceeded counts, and false display guards. Reusable `--thresholds-file` policies and CLI `--max-*` overrides may apply only to aggregate abuse counters, and `--fail-on-threshold` may return exit code 3 after preserving stdout. It must not output raw tokens, admin tokens, token hashes, private keys, session ids, payloads, ciphertext bodies, arbitrary frame contents, message text, stream chunks, room-event plaintext, threshold policy contents, or manifest contents. It is still an operator snapshot/script gate, not hosted billing, adaptive abuse automation, distributed alerting, distributed locking, or distributed retention orchestration.
