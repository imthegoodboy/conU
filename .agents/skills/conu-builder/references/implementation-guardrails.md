...
- `conu-relay --mailbox-audit` may scan `CONU_RELAY_MAILBOX_DIR` durable mailbox files for aggregate retention metadata only. It may report mailbox file counts, durable byte totals, oldest/newest queued timestamps, optional expired counts under an operator-supplied TTL, invalid mailbox-file counts, optional validated node id filters, and false display guards. `conu-relay --mailbox-purge` may delete only expired valid `.mailbox` files after a positive TTL and explicit `--confirm`, or report the same aggregate expired set without deleting when `--dry-run` is used. Both commands must validate optional node filters and must not print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, private keys, session ids, plaintext payloads, message text, stream chunks, room-event plaintext, or arbitrary mailbox file contents. These commands are single-relay operator workflows, not distributed hosted retention, billing, adaptive automation, or scheduled hosted purge orchestration.
...
- A local release may be marked ready only with documented limits. Do not claim public hosted internet readiness until distributed multi-instance session migration, distributed hosted accounting dashboards beyond local dashboard snapshots, scheduled/distributed hosted mailbox retention orchestration beyond local mailbox audit/purge workflows, distributed tenant lifecycle, hosted dashboard permission workflows, and managed hosted key administration are implemented.
