...
- npm install scripts may download release archives and copy binaries, but must require HTTPS except loopback HTTP smoke servers, reject embedded download URL credentials, avoid printing URL query strings or fragments in download errors, preflight archive member names and unsupported link entries before extraction, and must not read, upload, log, or package local conU state.
