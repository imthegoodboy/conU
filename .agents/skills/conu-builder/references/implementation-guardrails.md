...
- `conu-relay --mailbox-audit` may scan `CONU_RELAY_MAILBOX_DIR` durable mailbox files for aggregate retention metadata only. It may report mailbox file counts, durable byte totals, oldest/newest queued timestamps, optional expired counts under an operator-supplied TTL, invalid mailbox-file counts, optional validated node id filters, and false display guards. It must not print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, private keys, session ids, plaintext payloads, message text, stream chunks, room-event plaintext, or arbitrary mailbox file contents. The current command is single-relay read-only visibility, not distributed hosted retention, automatic purge, billing, or adaptive workflow automation.
...
- A local release may be marked ready only with documented limits. Do not claim public hosted internet readiness until distributed multi-instance session migration, distributed hosted accounting dashboards beyond local dashboard snapshots, automated hosted mailbox retention/purge policy beyond local mailbox audit snapshots, distributed tenant lifecycle, hosted dashboard permission workflows, and managed hosted key administration are implemented.
