...
- `conu-relay --mailbox-audit` may scan `CONU_RELAY_MAILBOX_DIR` durable mailbox files for aggregate retention metadata only. It may report mailbox file counts, durable byte totals, oldest/newest queued timestamps, optional expired counts under an operator-supplied TTL, invalid mailbox-file counts, optional validated node id filters, and false display guards. `conu-relay --mailbox-purge` may delete only expired valid `.mailbox` files after a positive TTL and explicit `--confirm`, or report the same aggregate expired set without deleting when `--dry-run` is used. `CONU_RELAY_MAILBOX_PURGE_INTERVAL_SECONDS` may enable relay-local scheduled deletion of expired valid `.mailbox` files using `CONU_RELAY_OFFLINE_ENVELOPE_TTL_SECONDS`; it must require `CONU_RELAY_MAILBOX_DIR` and must not print stored file contents. Audit, manual purge, and scheduled purge must not print stored relay frames, ciphertext bodies, raw tokens, token hashes, admin tokens, private keys, session ids, plaintext payloads, message text, stream chunks, room-event plaintext, or arbitrary mailbox file contents. These are single-relay operator workflows, not distributed hosted retention, billing, adaptive automation, or tenant-wide retention orchestration.
