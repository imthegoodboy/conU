...
- `conu-relay --hosted-fleet-dashboard` may aggregate a versioned manifest of multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse stores. The manifest must require explicit false display guards; relative paths resolve from the manifest directory; output may include only source paths, relay names, filters, aggregate counters, optional mailbox retention check counts/exceeded source counts, optional aggregate abuse threshold check counts/exceeded counts, and false display guards. Reusable mailbox `--retention-policy-file` policies and CLI `--ttl-seconds` overrides may apply only to relay-local mailbox metadata scans; CLI TTL overrides all fleet mailbox TTLs for that run, while per-relay `mailbox_ttl_seconds` remains a source-specific override ahead of the policy-file TTL. `--fail-on-retention` may return exit code 3 after preserving stdout only when TTL-checked mailbox sources report expired records. Reusable `--thresholds-file` policies and CLI `--max-*` overrides may apply only to aggregate abuse counters, and `--fail-on-threshold` may return exit code 3 after preserving stdout. It must not output raw tokens, admin tokens, token hashes, private keys, session ids, payloads, ciphertext bodies, arbitrary frame contents, message text, stream chunks, room-event plaintext, mailbox file contents, threshold/retention policy contents, or manifest contents, and it must not purge files or contact remote relays. It is still an operator snapshot/script gate, not hosted billing, adaptive abuse automation, distributed alerting, distributed locking, remote purge, or distributed retention orchestration.
...
- A local release may be marked ready only with documented limits. Do not claim public hosted internet readiness until distributed multi-instance session migration, distributed hosted accounting dashboards beyond local/admin-gated single-relay and guarded fleet dashboard snapshots, distributed hosted mailbox retention orchestration beyond read-only fleet retention gates plus local/admin-gated mailbox audit/purge and relay-local scheduled purge workflows, distributed tenant lifecycle/workflow automation beyond single-relay account suspension/scoped admin tokens, and managed hosted key administration are implemented.
