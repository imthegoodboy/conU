...
- Aggregate multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse metadata stores with `conu-relay --hosted-fleet-dashboard --fleet-file <path> [--account <id>] [--node <id>] [--json]`.
...
- `conu-relay --hosted-fleet-dashboard --fleet-file <path>` can aggregate multiple relay-local metadata stores from a guarded manifest, but it is still an operator snapshot, not hosted billing, distributed alerting, adaptive abuse response, or distributed retention orchestration.
