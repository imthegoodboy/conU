...
For guarded local fleet account workflow checks, run `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file C:\conu-relay\fleet.toml --json --fail-on-warning` to report credential/tenant source coverage, missing account records, and active/revoked consistency warning categories without mutating local files. To suspend, run `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file C:\conu-relay\fleet.toml --dry-run --json`, review the aggregate and per-relay credential/tenant counts, then rerun with `--confirm`. Each affected relay entry must include both `credentials_file` and `tenants_file`; partial entries fail closed for suspension, every complete source is preflighted before confirmed mutation, tenant metadata is revoked before account credential records, output is metadata-only, and no remote relay is contacted.
