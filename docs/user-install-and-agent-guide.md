...
- Aggregate multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse metadata stores with `conu-relay --hosted-fleet-dashboard --fleet-file <path> [--account <id>] [--node <id>] [--ttl-seconds <seconds>] [--retention-policy-file <path>] [--thresholds-file <path>] [--max-<metric> <count>...] [--json] [--fail-on-threshold] [--fail-on-retention]`, and turn aggregate fleet abuse threshold breaches into deterministic operator categories with `conu-relay --hosted-fleet-abuse-response-plan --fleet-file <path> [--node <id>] [--thresholds-file <path>] [--max-<metric> <count>...] [--json] [--fail-on-action]`.
