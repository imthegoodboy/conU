...
`conu routes sync` writes only route metadata. A direct route is selected only after a live QUIC probe succeeds and the remote node answers a peer-encrypted challenge using the trusted peer-card key. Failed direct probes are recorded with `direct_quic_probe_failed`, and relay remains selected. The command does not print, log, or inspect message contents.
...
`CONU_RELAY_SESSION_STATE_DIR` writes per-node `.session` files with node ids, relay session ids, timestamps, and display guards only. These files are useful for self-hosted relay restart recovery, but they are not a distributed session migration or lock service. `CONU_RELAY_ACCOUNTING_DIR` writes per-node `.accounting` files with metadata counters only. These files are useful for self-hosted usage checks and quota enforcement, but they are not a managed billing, abuse-monitoring, or hosted dashboard system.
