...
- Use the public npm launcher package backed by platform release assets and checksums.
- Verify tagged Linux release assets with SHA-256 sidecars, GitHub attestations, `conu-linux-gpg-key.asc`, the published full maintainer fingerprint, detached `.asc` signatures, and native APT/RPM repository metadata signatures when those assets are published.
- Host the current relay yourself for controlled `ws://` tests or behind TLS termination for `wss://`.
- Aggregate multiple relay-local credential, tenant, session-state, mailbox, accounting, and abuse metadata stores with `conu-relay --hosted-fleet-dashboard --fleet-file <path> [--account <id>] [--node <id>] [--ttl-seconds <seconds>] [--retention-policy-file <path>] [--thresholds-file <path>] [--max-<metric> <count>...] [--json] [--fail-on-threshold] [--fail-on-retention]`, turn aggregate fleet abuse threshold breaches into deterministic operator categories with `conu-relay --hosted-fleet-abuse-response-plan --fleet-file <path> [--node <id>] [--thresholds-file <path>] [--max-<metric> <count>...] [--json] [--fail-on-action]`, audit guarded local fleet account or account/node credential/tenant consistency with `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file <path> [--node <id>] [--json] [--fail-on-warning]`, dry-run or confirm guarded local fleet account/node credential revocation with `conu-relay --hosted-fleet-credential-revoke <account-id> <node-id> --fleet-file <path> (--dry-run|--confirm) [--json]`, dry-run or confirm guarded local fleet tenant account lifecycle with `conu-relay --hosted-fleet-tenant-upsert <account-id> --fleet-file <path> (--dry-run|--confirm) [--json]` and `conu-relay --hosted-fleet-tenant-revoke <account-id> --fleet-file <path> (--dry-run|--confirm) [--json]`, and dry-run or confirm guarded local fleet account or account/node suspension with `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> [--node <id>] (--dry-run|--confirm) [--json]`.

## What Does Not Work Yet

- No signed one-click installer yet.
- The exact unscoped `conu` npm package is not available; install `@imthegoodboy/conu`, which still exposes the `conu` command.
...
- Relay-backed one-shot message, stream-chunk, room-event, and bounded offline-mailbox delivery exists through the conUD relay pump with a reusable daemon relay session and same-node reconnect resume; set `CONU_RELAY_SESSION_STATE_DIR` on the relay for metadata-only same-node resume records across relay restarts and `CONU_RELAY_MAILBOX_DIR` for ciphertext persistence across relay restarts. Relays can use offline scoped credential issuance, manifest upsert/rotation/revocation helpers, account-scoped online issue/rotate/revoke/audit, scoped hosted admin-token manifests with payload-safe local `--admin-token-audit`, payload-safe local `--hosted-readiness` startup/release preflights, admin-gated online tenant account/node lifecycle for one configured relay registry, local/admin-gated hosted account suspension, guarded local fleet account/node audit, credential revoke, tenant account upsert/revoke, tenant-node upsert/revoke, account/node suspension, live-reloaded hashed scoped credential manifests with revocation/expiry metadata, payload-safe local and admin-gated online session-state audits, payload-safe local and admin-gated online mailbox retention audits with reusable policy files, confirm-gated local/admin and guarded local fleet expired mailbox purge, relay-local scheduled expired mailbox purge, metadata-only accounting with optional sent quotas, metadata-only abuse counters, local/admin-gated abuse threshold reports with reusable policy files and optional fail-on-threshold exit status, local hosted-dashboard snapshots, guarded hosted-fleet-dashboard snapshots, guarded hosted-fleet abuse response plans, guarded hosted-fleet mailbox purge, and admin-gated online hosted-dashboard snapshots. Distributed hosted dashboards/accounting/adaptive abuse automation beyond single-relay threshold reports, readiness preflights, and guarded fleet response plans; remote/distributed tenant lifecycle/workflow services beyond guarded local fleet account/node audit, credential revoke, tenant account lifecycle, tenant-node lifecycle, account/node suspension plus scoped single-relay account suspension/admin tokens; remote relay/cross-region mailbox retention orchestration beyond guarded local fleet cleanup; and distributed multi-instance session migration are not implemented yet.
- `conu-relay --hosted-fleet-dashboard --fleet-file <path>` can aggregate multiple relay-local metadata stores from a guarded manifest and apply reusable mailbox retention policies plus reusable abuse thresholds to aggregate counters. `conu-relay --hosted-fleet-abuse-response-plan --fleet-file <path>` can map aggregate fleet abuse threshold breaches to static operator action categories and optional `--fail-on-action` script status. `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file <path> [--node <node-id>]` can report credential/tenant source coverage and account-wide or node-scoped consistency warning categories before mutation. `conu-relay --hosted-fleet-credential-revoke <account-id> <node-id> --fleet-file <path>` can dry-run or confirm credential-only account/node revocation across configured local credential manifests after preflighting every source. `conu-relay --hosted-fleet-tenant-upsert <account-id> --fleet-file <path>` and `conu-relay --hosted-fleet-tenant-revoke <account-id> --fleet-file <path>` can dry-run or confirm tenant account lifecycle across configured local tenant registries after preflighting every source. `conu-relay --hosted-fleet-tenant-node-upsert <account-id> <node-id> --fleet-file <path>` and `conu-relay --hosted-fleet-tenant-node-revoke <account-id> <node-id> --fleet-file <path>` can dry-run or confirm tenant-node metadata lifecycle across configured local tenant registries after preflighting every source. `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> [--node <node-id>]` can dry-run or confirm tenant-first account suspension, or tenant-node-first node suspension, across complete local credential/tenant source pairs. `conu-relay --hosted-fleet-mailbox-purge --fleet-file <path>` can dry-run or confirm expired valid `.mailbox` cleanup across manifest-listed local stores. These are still operator snapshot/script/response-plan/local-audit/credential-revoke/account-lifecycle/node-lifecycle/suspension/cleanup gates, not hosted billing, distributed alerting, adaptive abuse response, remote tenant control, remote purge, cross-region retention locking, or distributed retention orchestration.
...
The npm package is not the conU implementation. It is a small launcher that downloads the native Rust archive for the user's platform with bounded request time and response sizes, verifies the `.sha256` checksum, rejects unsafe archive member lists before extraction, bounds post-extraction binary selection, and exposes `conu`, `conud`, `conu-relay`, and `conu-mcp` on `PATH`. For local package testing, `CONU_NPM_BINARY_DIR` must point to an existing directory containing regular files for every expected binary.
...
`CONU_RELAY_SESSION_STATE_DIR` writes per-node `.session` files with node ids, relay session ids, timestamps, and display guards only. These files are useful for self-hosted relay restart recovery, but they are not a distributed session migration or lock service. Inspect local session-state metadata with `conu-relay --session-audit --session-state-dir C:\conu-relay\sessions [--node <node-id>] --json`; managed relay operators can query the same metadata from the running relay with `conu-relay --admin-session-audit --relay wss://relay.example.com/conu --admin-token-stdin [--node <node-id>] --json`. Both forms report counts, active/expired/invalid totals, timestamp bounds, and false display guards only, not relay session ids or file contents. `CONU_RELAY_MAILBOX_DIR` writes durable `.mailbox` files for peer-encrypted offline envelopes. Inspect local mailbox retention pressure with `conu-relay --mailbox-audit --mailbox-dir C:\conu-relay\mailbox --ttl-seconds 3600 --retention-policy-file C:\conu-relay\mailbox-retention.toml --json`; audit output reports file counts, byte totals, timestamp bounds, optional expired counts, invalid mailbox-file counts, and display guards only, not stored frames or ciphertext bodies. Managed relay operators can query the same read-only retention metadata from the running relay with `conu-relay --admin-mailbox-audit --relay wss://relay.example.com/conu --admin-token-stdin --ttl-seconds 3600 --retention-policy-file C:\conu-relay\mailbox-retention.toml --json`. To remove expired durable mailbox files, run `conu-relay --mailbox-purge --mailbox-dir C:\conu-relay\mailbox --retention-policy-file C:\conu-relay\mailbox-retention.toml --dry-run --json`, review the aggregate counts, then rerun with `--confirm`; managed relay operators can run the same cleanup against the running relay with `conu-relay --admin-mailbox-purge --relay wss://relay.example.com/conu --admin-token-stdin --retention-policy-file C:\conu-relay\mailbox-retention.toml (--dry-run|--confirm) --json`. For a local fleet manifest with several relay mailbox stores, run `conu-relay --hosted-fleet-mailbox-purge --fleet-file C:\conu-relay\fleet.toml --retention-policy-file C:\conu-relay\mailbox-retention.toml --dry-run --json`, review aggregate and per-relay expired counts, then rerun with `--confirm`. Confirmed purge deletes only expired valid `.mailbox` files and still reports only aggregate/per-relay metadata. Account-scoped admin session and mailbox tokens require a node filter and an active tenant-node record. Set `CONU_RELAY_MAILBOX_PURGE_INTERVAL_SECONDS` with `CONU_RELAY_MAILBOX_DIR` to run the same expired valid-file cleanup on a relay-local schedule using the offline envelope TTL. `CONU_RELAY_ACCOUNTING_DIR` writes per-node `.accounting` files with metadata counters only. `CONU_RELAY_ABUSE_DIR` writes `.abuse` files with aggregate denial/enforcement counters for unauthorized sessions, credential or tenant denies, rate limits, session expiry, quota denies, undelivered forwards, and mailbox rejects. Inspect them with `conu-relay --abuse-audit --abuse-dir C:\conu-relay\abuse --json`, compare explicit maximums with `conu-relay --abuse-threshold-report --abuse-dir C:\conu-relay\abuse --max-admin-unauthorized 0 --max-rate-limited-sessions 100 --json --fail-on-threshold`, inspect a combined local operator snapshot with `conu-relay --hosted-dashboard --credentials-file C:\conu-relay\credentials.toml --tenants-file C:\conu-relay\tenants.toml --accounting-dir C:\conu-relay\accounting --abuse-dir C:\conu-relay\abuse --json`, run a pre-startup/release check with `conu-relay --hosted-readiness --bind-addr 0.0.0.0:8787 --credentials-file C:\conu-relay\credentials.toml --admin-tokens-file C:\conu-relay\admin-tokens.toml --tenants-file C:\conu-relay\tenants.toml --session-state-dir C:\conu-relay\sessions --mailbox-dir C:\conu-relay\mailbox --retention-policy-file C:\conu-relay\mailbox-retention.toml --accounting-dir C:\conu-relay\accounting --abuse-dir C:\conu-relay\abuse --thresholds-file C:\conu-relay\abuse-thresholds.toml --max-rate-limited-sessions 100 --json --fail-on-warning`, plan fleet abuse response categories with `conu-relay --hosted-fleet-abuse-response-plan --fleet-file C:\conu-relay\fleet.toml --thresholds-file C:\conu-relay\abuse-thresholds.toml --json --fail-on-action`, suspend one account with `conu-relay --hosted-account-suspend <account-id> --credentials-file C:\conu-relay\credentials.toml --tenants-file C:\conu-relay\tenants.toml --json`, or query a running managed relay with `conu-relay --admin-hosted-dashboard --relay wss://relay.example.com/conu --admin-token-stdin --json`. Managed operators can also run `conu-relay --admin-abuse-threshold-report --relay wss://relay.example.com/conu --admin-token-stdin --max-admin-unauthorized 0 --max-rate-limited-sessions 100 --json --fail-on-threshold` to get count/max/exceeded metadata from the running relay. With `--fail-on-threshold`, threshold reports preserve stdout and return exit code 3 only when a configured maximum is exceeded; with `--fail-on-warning`, hosted-readiness preserves stdout and returns exit code 3 when warnings exist, including exceeded configured abuse thresholds; with `--fail-on-action`, hosted fleet abuse response plans preserve stdout and return exit code 3 only when a threshold breach maps to at least one action category. Audit, threshold, dashboard, readiness, and response-plan output does not show tokens, token hashes, admin tokens, private keys, session ids, payloads, ciphertext bodies, frame contents, or manifest contents. Account-scoped dashboard tokens without a node filter suppress global accounting and abuse counters. These files and snapshots are useful for self-hosted usage checks, session-state visibility, quota enforcement, mailbox pressure checks, local/admin/fleet expired-mailbox cleanup, relay-local tenant lifecycle/account suspension, and relay-local abuse visibility, but they are not a managed billing, distributed abuse-monitoring, adaptive enforcement service, remote relay retention purge, cross-region retention locking, or distributed hosted dashboard system.

For guarded local fleet account workflow checks, run `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file C:\conu-relay\fleet.toml [--node <node-id>] --json --fail-on-warning` to report credential/tenant source coverage, missing account or node records, and active/revoked consistency warning categories without mutating local files. With `--node`, the audit narrows credential and tenant-node counts to the requested node and may return node-specific warning categories. To revoke only a compromised credential across manifest-listed local credential stores, run `conu-relay --hosted-fleet-credential-revoke <account-id> <node-id> --fleet-file C:\conu-relay\fleet.toml --dry-run --json`, review the aggregate and per-relay credential counts, then rerun with `--confirm`; every credential source is preflighted before confirmed mutation and account/node collisions or duplicate node records fail closed. To add or revoke the tenant account itself across manifest-listed local tenant registries, run `conu-relay --hosted-fleet-tenant-upsert <account-id> --fleet-file C:\conu-relay\fleet.toml --dry-run --json` or `conu-relay --hosted-fleet-tenant-revoke <account-id> --fleet-file C:\conu-relay\fleet.toml --dry-run --json`, review tenant/source counts, then rerun the selected command with `--confirm`; every tenant source is preflighted, upsert may create missing tenant files, and revoke requires the account to already exist. To suspend, run `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file C:\conu-relay\fleet.toml [--node <node-id>] --dry-run --json`, review the aggregate and per-relay credential/tenant counts, then rerun with `--confirm`. Each affected relay entry must include both `credentials_file` and `tenants_file`; partial entries fail closed for suspension, every complete source is preflighted before confirmed mutation, account-wide mode revokes tenant metadata before account credential records, node mode revokes tenant-node metadata before matching node credential records, output is metadata-only, and no remote relay is contacted.
...
"reply bytes" | conu messages reply agent.b <envelope-id> --stdin
conu watch
conu stop
```
...
- Distributed hosted session lifecycle beyond local/admin-gated single-relay `--session-audit` snapshots, distributed hosted dashboards/accounting beyond local/admin-gated single-relay `--hosted-dashboard`, `--hosted-readiness`, `--hosted-fleet-dashboard`, guarded `--hosted-fleet-abuse-response-plan`, `--admin-hosted-dashboard`, `--abuse-threshold-report`, and `--admin-abuse-threshold-report` snapshots/reports with optional script exit status, remote relay/cross-region mailbox retention orchestration beyond guarded hosted fleet cleanup, local/admin-gated `--mailbox-audit`, local/admin-gated `--mailbox-purge`, guarded local `--hosted-fleet-mailbox-purge`, and scheduled purge workflows, and remote/distributed tenant lifecycle/workflow automation beyond guarded local `--hosted-fleet-account-audit` account/node checks, guarded local `--hosted-fleet-credential-revoke`, guarded local `--hosted-fleet-tenant-upsert`/`--hosted-fleet-tenant-revoke`, guarded local `--hosted-fleet-tenant-node-upsert`/`--hosted-fleet-tenant-node-revoke`, guarded local `--hosted-fleet-account-suspend` account/node suspension, the single-relay account credential, scoped admin-token manifest and audit, hosted account suspension, and local/admin-gated tenant registry lifecycle.
- Broader OS package-manager submission and hosted update repository publication beyond npm.
