...
For repeated mailbox retention checks or cleanup, create a metadata-only retention policy file and pass `--retention-policy-file C:\conu-relay\mailbox-retention.toml` to local or admin mailbox audit/purge commands. The file uses `version = "1"`, optional `ttl_seconds` and `node_id` keys, and false display guards; CLI `--ttl-seconds` and `--node` options can override it for a single run. For repeated threshold checks, create a metadata-only thresholds file and pass `--thresholds-file C:\conu-relay\abuse-thresholds.toml` to either local or admin threshold command. The file uses `version = "1"`, supported `max_*` keys, and false display guards; CLI `--max-*` options can override it for a single run.
