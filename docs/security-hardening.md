...
Private keys are never printed by CLI output. On Windows, new security key files wrap local signing, exchange, and storage secret bytes with current-user DPAPI (`secretStorageBackend = "windows-dpapi-user"`), and `conu security audit` migrates older plaintext-hex key files to wrapped fields when it can read them. On Unix targets, key files are created with owner-only permissions and still need a platform keychain, Secure Enclave, HSM, or user-managed secret backend before high-security public release claims.
...
The credential file is `security/relay-credential.key`. On Windows it stores `token_dpapi_hex` with current-user DPAPI wrapping; on non-Windows it stores an owner-only local secret file until platform keychain support lands. CLI and MCP status surfaces report only configured/backend/protection booleans and never print the token or protected blob.
...
- Non-Windows OS keychain, Secure Enclave, HSM, or user-managed secret backend for private key protection.
...
- Non-Windows OS-backed relay secret storage and hosted retention/accounting policy beyond the current durable ciphertext relay mailbox files and metadata-only relay accounting counters.
