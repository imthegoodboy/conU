...
- Hosted retention/accounting policy and managed key administration beyond the current local secret storage backends, account-scoped relay credential lifecycle, durable ciphertext relay mailbox files, payload-safe relay mailbox audit/purge workflows with reusable retention policy files, guarded local hosted fleet mailbox purge, guarded local hosted fleet account/node audit, tenant-node lifecycle, and suspension, guarded local hosted fleet abuse response plans, payload-safe hosted-readiness reuse of retention and threshold policy files, metadata-only relay accounting counters, metadata-only relay abuse counters, hosted fleet retention gates over relay-local mailbox metadata, and single-relay/fleet abuse threshold reports with reusable policy files and optional script exit status.
