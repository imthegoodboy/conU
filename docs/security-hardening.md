...
## Relay Session-State Audit

`CONU_RELAY_SESSION_STATE_DIR` can persist metadata-only same-node relay resume records. The files may contain node ids, relay session ids, timestamps, and display guards, but not relay tokens, token hashes, payloads, ciphertext bodies, private keys, or account secrets.

Operators can inspect that store with `conu-relay --session-audit --session-state-dir <path> [--node <node-id>] [--json]`. Managed relay operators can query the running relay with `conu-relay --admin-session-audit --relay <endpoint> --admin-token-stdin [--node <node-id>] [--json]` when file-backed session state is configured. Both forms report only record counts, active/expired/invalid totals, timestamp bounds, and false display guards. The admin form uses the hosted admin token boundary; scoped admin-token manifests grant this with `scope_sessions = true`, and account-scoped tokens require a node filter plus an active tenant-node record.

Session audit output must not print relay session ids, raw node tokens, token hashes, admin tokens, payloads, ciphertext bodies, private keys, arbitrary frame contents, or session-state file contents.
...
- Distributed multi-instance relay session migration beyond local/admin-gated `conu-relay --session-audit` snapshots, distributed hosted dashboards/accounting/adaptive abuse automation beyond local `conu-relay --hosted-dashboard`, manifest-driven `conu-relay --hosted-fleet-dashboard` with aggregate reusable mailbox retention checks plus aggregate reusable abuse threshold checks, guarded `conu-relay --hosted-fleet-abuse-response-plan` operator categories, policy-aware local `conu-relay --hosted-readiness`, local `conu-relay --abuse-threshold-report`, admin-gated `conu-relay --admin-hosted-dashboard`, and admin-gated `conu-relay --admin-abuse-threshold-report` snapshots/reports with reusable threshold policy files and optional script exit status, remote relay/cross-region hosted relay mailbox retention orchestration beyond guarded local fleet cleanup, local/admin-gated `conu-relay --mailbox-audit`, local/admin-gated `conu-relay --mailbox-purge`, guarded local `conu-relay --hosted-fleet-mailbox-purge`, reusable mailbox retention policy files, and `CONU_RELAY_MAILBOX_PURGE_INTERVAL_SECONDS` workflows, and remote/distributed tenant lifecycle/full hosted identity administration around the current single-relay account credential, scoped admin-token manifest plus payload-safe `conu-relay --admin-token-audit`, local/admin-gated tenant registry lifecycle, hosted account suspension, guarded local `conu-relay --hosted-fleet-account-audit` account/node checks, guarded local `conu-relay --hosted-fleet-credential-revoke`, guarded local `conu-relay --hosted-fleet-tenant-upsert`/`--hosted-fleet-tenant-revoke`, guarded local `conu-relay --hosted-fleet-tenant-node-upsert`/`--hosted-fleet-tenant-node-revoke`, and guarded local `conu-relay --hosted-fleet-account-suspend` account/node suspension.
- SDK/MCP permission hardening for multi-tenant or hosted deployments.
- Hosted retention/accounting policy and managed key administration beyond the current local secret storage backends, account-scoped relay credential lifecycle, durable ciphertext relay mailbox files, payload-safe relay mailbox audit/purge workflows with reusable retention policy files, guarded local hosted fleet mailbox purge, guarded local hosted fleet account/node audit, credential revoke, tenant account lifecycle, tenant-node lifecycle, and suspension, guarded local hosted fleet abuse response plans, payload-safe hosted-readiness reuse of retention and threshold policy files, metadata-only relay accounting counters, metadata-only relay abuse counters, hosted fleet retention gates over relay-local mailbox metadata, and single-relay/fleet abuse threshold reports with reusable policy files and optional script exit status.
