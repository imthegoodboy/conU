...
- Homebrew, Scoop, winget, Chocolatey, Debian, and RPM package-manager files are generated from verified release assets and checksums on tagged releases. CI and release package checks also build the generated RPM spec and optional unsigned RPM release assets with native `rpmbuild` when RPM tools are available. Submitting those generated files to package-manager tap/bucket/repository feeds, plus detached Linux package signatures and auto-update policy, can come after the first signed archive/npm release is stable.
...
The release workflow builds platform-named artifacts and uploads matching checksum files. Tagged release builds run a fail-closed preflight before package checks and platform builds; that preflight requires maintainer-owned signing secrets for Windows Authenticode, macOS Developer ID/notarization, and the repository `NPM_TOKEN` used for npm provenance publication. Linux archives use SHA-256 files plus GitHub artifact attestations, and generated Debian/RPM packages use SHA-256 sidecars, until native distro package signing is added. Tagged GitHub Release publication also generates package-native `conu.rb`, `conu.json`, `imthegoodboy.conU.yaml`, `conu.<version>.nupkg`, `conu_<version>_amd64.deb`, `conu_<version>_arm64.deb`, `conu.spec`, unsigned `conu-<rpm-version>-1.x86_64.rpm`, and unsigned `conu-<rpm-version>-1.aarch64.rpm` files from the verified asset checksums so package-manager metadata and Linux package files are not hand-edited; package gates run native `rpmbuild` validation before those tag paths are used.
...
packages with `.rpm.sha256` sidecars. The regression check validates generated
Debian packages with `dpkg-deb`, builds the RPM spec with `rpmbuild`, and checks
generated RPM assets when those tools are installed. It does not read local conU
state, tokens, signing material, or package-manager repository credentials, and
it still does not sign `.rpm` assets or configure an RPM repository.
...
5. Submit generated Homebrew/Scoop/winget/Chocolatey/Debian/RPM files to the appropriate package-manager repositories, then add detached Linux package signatures, RPM package signing, repository metadata, and auto-update policy after npm and signed release archives are stable.
