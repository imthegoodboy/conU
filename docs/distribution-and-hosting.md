...
4. Configure the repository signing secrets, `CONU_LINUX_GPG_KEY_FINGERPRINT`, and `NPM_TOKEN`; before storing Windows/macOS values, run `python scripts/check-platform-signing-secrets-preflight.py --require-openssl --json`, then run `python scripts/check-tagged-release-readiness.py --repo imthegoodboy/conU --tag v0.1.0 --npm-registry-check`; the commands fail before tag creation if platform signing values, required secret names, default Pages or custom S3 repository settings, GitHub Release clobber status, package version/tag consistency, or target npm package availability are not ready.
