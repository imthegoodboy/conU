...
- Managed hosted mailbox retention/accounting dashboards beyond the current self-hosted durable ciphertext files, metadata-only mailbox counters, and relay-local mailbox audit snapshots.
...
4. Add distributed account control planes, distributed monitoring/dashboards, automated hosted mailbox retention/purge policy, and distributed multi-instance session migration before opening a managed relay to everyone.
