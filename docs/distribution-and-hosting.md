...
operator-controlled static HTTPS hosting. The regression checks
...
validate the hosted repository bundle and site layouts plus fail-closed
signature/base-URL checks. They do not read local conU state, tokens, signing
material, or package-manager repository credentials, and they still do not
deploy DNS/TLS/static hosting infrastructure or submit package-manager
repository PRs.
...
5. Submit generated Homebrew/Scoop/winget/Chocolatey/Debian/RPM files to the appropriate package-manager repositories, deploy the signed hosted Linux repository site artifact to an operator-controlled static HTTPS endpoint with DNS/TLS/cache policy, then add auto-update policy after npm and signed release archives are stable.
