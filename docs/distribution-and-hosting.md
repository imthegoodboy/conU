...
bundles against the actual package hashes, verify native RPM package signing
with an ephemeral GPG key when RPM signing tools are available, and verify
native repository signatures with an ephemeral GPG key. They do not read local
conU state, tokens, signing material, or package-manager repository
credentials, and they still do not configure hosted package repositories.
...
5. Submit generated Homebrew/Scoop/winget/Chocolatey/Debian/RPM files to the appropriate package-manager repositories, then add hosted APT/RPM repository publication and auto-update policy after npm and signed release archives are stable.
