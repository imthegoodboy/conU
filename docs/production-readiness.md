...
- Account-scoped online relay credential issue, rotate, revoke, and audit commands through `CONU_RELAY_ADMIN_TOKEN` or hashed scoped entries in `CONU_RELAY_ADMIN_TOKENS_FILE` plus `CONU_RELAY_CREDENTIALS_FILE`, sending only node-token hash metadata to the relay and returning metadata-only admin results.
