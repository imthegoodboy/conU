...
conu-relay --hosted-readiness --credentials-file <path> --admin-tokens-file <path> --tenants-file <path> --session-state-dir <path> --mailbox-dir <path> --accounting-dir <path> --abuse-dir <path> --json --fail-on-warning
git diff --check
```
...
- Hosted relay readiness output reports only configured paths, source booleans, counts, warnings, bind metadata, and false display guards; it must not print raw tokens, token hashes, admin tokens, private keys, relay session ids, payloads, ciphertext bodies, frame contents, or manifest contents.
