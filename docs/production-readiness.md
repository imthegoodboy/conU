...
- Metadata-only relay session state through `CONU_RELAY_SESSION_STATE_DIR`, relay accounting through `CONU_RELAY_ACCOUNTING_DIR`, and relay abuse counters through `CONU_RELAY_ABUSE_DIR`, with per-node session, sent/received byte, envelope, mailbox, and enforcement counters plus optional per-window sent-envelope and sent-byte quotas.
