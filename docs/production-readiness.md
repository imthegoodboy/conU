...
- Hosted readiness also accepts mailbox `--retention-policy-file`, abuse `--thresholds-file`, and inline `--max-*` overrides when the matching mailbox or abuse store is configured. Its report includes threshold checks/exceeded counts without rendering policy contents or sensitive relay data, and exceeded thresholds contribute to warning status and `--fail-on-warning`.
...
- Distributed multi-instance relay session migration/accounting beyond local/admin-gated single-relay session-state audits, distributed hosted accounting dashboards beyond local/admin-gated single-relay dashboard and threshold-report snapshots, hosted retry policy, distributed hosted mailbox retention orchestration beyond local/admin-gated audit/purge plus local scheduled purge workflows, distributed tenant lifecycle beyond single-relay account suspension/scoped admin tokens, and managed mailbox retention dashboards beyond single-relay audit snapshots.
