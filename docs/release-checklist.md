...
- Linux release tarballs use SHA-256 checksum files plus GitHub artifact attestations, and generated Debian/RPM packages use SHA-256 sidecars, until distro package signing exists:
...
- CI and release package jobs install RPM tooling and run `scripts/check-package-manager-manifests.py` before artifact builds, so generated package-manager manifest, RPM spec build, and optional RPM asset regressions fail before tag publication paths are used.
...
- GitHub Release has generated `conu.rb`, `conu.json`, `imthegoodboy.conU.yaml`, `conu.<version>.nupkg`, `conu_<version>_amd64.deb`, `conu_<version>_arm64.deb`, `.deb.sha256` sidecars, `conu.spec`, unsigned `conu-<rpm-version>-1.x86_64.rpm`, unsigned `conu-<rpm-version>-1.aarch64.rpm`, and `.rpm.sha256` sidecars derived from the same strict release checksums before package-manager submission.
