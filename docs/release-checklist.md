...
- Run `python scripts/check-tagged-release-readiness.py --repo <owner/name> --tag v<version> --npm-registry-check --require-ci` before creating a release tag; it combines live release-secret names, default Pages or custom S3 repository settings, GitHub Release clobber status, package version/tag consistency, optional npm registry conflict checks, and the target commit's latest `CI` workflow status without printing secret values, release notes, asset URLs, logs, or variable values.
