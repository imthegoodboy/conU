...
- Run `python scripts/check-rpm-package-signing.py`; CI and release package gates use the same regression check to prove generated RPM package payloads can be signed with the Linux GPG key, verified with native RPM tooling, refreshed with strict `.rpm.sha256` sidecars, and used to generate RPM repository metadata.
...
- CI and release package jobs install RPM tooling plus `createrepo-c` and `gnupg`, run `scripts/check-package-manager-manifests.py`, `scripts/check-rpm-package-signing.py`, `scripts/check-linux-release-signing.py`, `scripts/check-linux-repository-signing.py`, and `scripts/check-linux-gpg-public-key-export.py` before artifact builds, so generated package-manager manifest, APT/RPM metadata, RPM spec build, optional RPM asset, native RPM package-signing, Linux detached-signing, native Linux repository-signing, and Linux public-key export regressions fail before tag publication paths are used.
