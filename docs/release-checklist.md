...
- `conu security audit` reports initialized local controls, the secret storage backend, and whether local secret bytes are OS-protected. On non-Windows, a configured user-managed wrap key should report `secretStorageBackend = "user-managed-wrap-key-v1"` while keeping `secretsOsProtected = false`.
...
Current decision target is `daemon_relay_message_stream_chunk_room_topic_policy_durable_mailbox_live_reloaded_hashed_relay_credential_manifest_lifecycle_accounting_quotas_session_resume_direct_route_selection_guard_log_rotation_identity_key_rotation_and_retirement_storage_key_rotation_and_retirement_windows_dpapi_nonwindows_user_managed_secret_wrap_key_stored_relay_client_credentials_local_capabilities_signed_agent_cards_peer_policy_auto_agent_card_exchange_and_attested_release_archives_ready_with_known_limits`. Public hosted/internet release remains blocked until hosted account auth, managed online credential issuance APIs, distributed hosted session state, distributed hosted dashboards/accounting, hosted mailbox retention policy, hosted multi-tenant permission administration, direct QUIC, and native non-Windows OS-backed key storage are finished.
