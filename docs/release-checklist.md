...
- Linux release tarballs use SHA-256 checksum files plus GitHub artifact attestations, and generated Debian/RPM packages plus unsigned APT metadata use SHA-256 sidecars, until distro package signing exists:
...
- CI and release package jobs install RPM tooling and run `scripts/check-package-manager-manifests.py` before artifact builds, so generated package-manager manifest, APT metadata, RPM spec build, and optional RPM asset regressions fail before tag publication paths are used.
