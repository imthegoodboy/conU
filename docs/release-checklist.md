...
- Run `python scripts/check-tagged-release-readiness.py --repo <owner/name> --tag v<version> --npm-registry-check` before creating a release tag; it combines live release-secret names, default Pages or custom S3 repository settings, GitHub Release clobber status, package version/tag consistency, and optional npm registry conflict checks without printing secret values, release notes, asset URLs, or variable values.
