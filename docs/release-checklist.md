...
- Run `python scripts/check-package-manager-manifests.py`; CI and release package gates use the same regression check to prove generated Homebrew/Scoop/winget/Chocolatey files require strict release checksums and safe Windows archive layouts.
...
- `scripts/generate-package-manager-manifests.py dist --output-dir dist --version <version> --tag v<version>` generates package-native `conu.rb`, `conu.json`, `imthegoodboy.conU.yaml`, and `conu.<version>.nupkg` files from platform release assets and strict checksum files; `scripts/check-package-manager-manifests.py` passes in CI/release package checks.
...
- GitHub Release has generated `conu.rb`, `conu.json`, `imthegoodboy.conU.yaml`, and `conu.<version>.nupkg` assets derived from the same strict release checksums before package-manager submission.
