...
- CI and release package jobs install RPM tooling and run `scripts/check-package-manager-manifests.py` before artifact builds, so generated package-manager manifest and RPM spec build regressions fail before tag publication paths are used.
