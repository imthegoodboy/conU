...
- Run `python scripts/set-github-release-secrets-regression.py`; CI and release package gates use the same regression check to prove release secret setup can generate an empty env-file template from the authoritative required-secret list, validates filled env-file completeness without GitHub access, reads local values from the environment or a strict ignored env file, supports env-file-only setup without environment fallback, passes env-file values to preflight subprocesses through environment variables, sends values to `gh secret set` through stdin rather than command arguments, and reports only secret names.
...
- Run `python scripts/check-npm-publish-preflight.py`; CI and release package checks validate publish metadata for `conu`, and tagged npm publication reruns it with `--registry-check --require-token-env NODE_AUTH_TOKEN` before the package is published.
...
- `scripts/verify-npm-package-contents.py` passes for npm package dry-run tarballs, proving they contain only the expected package files and no local state, build output, vendored native binaries, secrets, or bundled dependencies.
...
- Generate that ignored local file with
  `python scripts/set-github-release-secrets.py --write-env-template .env.release`
  so it starts from the authoritative required-secret list; the command writes
  empty values only and refuses to overwrite an existing file.
- Run
...
- CI, release package checks, and tagged npm publication run `scripts/verify-npm-package-contents.py` so package dry-runs fail closed on missing required files, unexpected files, forbidden state/build/payload paths, oversized files, or bundled dependencies.
- CI and release package checks run `scripts/check-npm-publish-preflight.py` and `scripts/check-npm-publish-preflight-regression.py`; tagged npm publication reruns the preflight with `--registry-check --require-token-env NODE_AUTH_TOKEN`, so an already-published `conu` version fails before publication starts.
...
- `conu` npm package content verification passes before publication.
- `conu` is published only after the matching GitHub Release assets are available; the tagged release preflight requires the repository `NPM_TOKEN` secret for automated npm publication with provenance, and the npm publish conflict preflight confirms the target package version is absent from npm before the publish command runs.
