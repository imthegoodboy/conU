...
Current latest hardening addition: release update apply backup-directory
allocation hardening is complete; PR #383 merged to `main` at merge commit
`dc3ce604c5306ac6639032d495331dfaa53d8c98`, Issue #382 is closed, and branch
`update-apply-backup-dir-allocation` is preserved. Validation: `cargo fmt --all -- --check`, `git diff --check`, release-version check, and PR CI passed.
Local executable Rust validation was attempted with both GNU and MSVC Windows
toolchains but this workstation is missing `dlltool.exe`/`link.exe`; PR CI
covered Rust on Ubuntu, macOS, and Windows. Prior completed hardening addition:
npm public package metadata gate and update staging temp-dir hardening are
