...
- Issue #196 tracks package-manager manifest generation hardening, and branch `package-manager-manifest-preflight` carries the implementation.
...
- Added `scripts/check-package-manager-manifests.py` with regression coverage for successful generation, rooted/rootless Windows layouts, missing checksum files, wrong checksum archive names, checksum mismatches, and forbidden local/secret output literals.
...
- Run PR CI plus branch `Release Artifacts`, merge only after green gates, verify main CI and main `Release Artifacts`, and preserve the `package-manager-manifest-preflight`, `npm-publish-conflict-preflight`, `release-artifact-smoke-binary-preflight`, `npm-smoke-local-binary-preflight`, and `npm-installer-local-binary-dir-guard` branches.
