...
- Issue #218 tracks this hardening slice on branch
  `linux-gpg-fingerprint-policy`; the branch must be preserved.
...
- Open PR for Issue #218, verify CI package jobs exercise native RPM signing
  with fingerprint enforcement on Ubuntu, run the branch `Release Artifacts`
  workflow, then merge without deleting the branch if checks pass.
