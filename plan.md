...
- Issue #250 tracks this hardening work on branch
  `published-update-download-gate`; the branch must be preserved after merge.
...
- Run package/readiness validation, open a PR, let CI and branch
  `Release Artifacts` pass, merge without deleting the branch, then exercise
  the new gate during the first real signed `v*` release.
