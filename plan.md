...
- Issue #242 is tracked by PR #243 on branch
  `release-update-policy-metadata`; the branch is preserved.
...
- conU still does not include an installed `conu update` client command that
  fetches and verifies this policy.
...
- Continue with real signing/npm secret configuration, a signed `v*` release,
  ...
  endpoint activation with generated cache policy application, a client-facing
  updater command that consumes the signed update policy, managed public relay
  hosting, distributed hosted dashboards/accounting/adaptive abuse automation,
