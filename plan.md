...
- Run post-merge `main` CI and the `Release Artifacts` workflow on `main`, then
  cut a real signed `v*` release and run `conu update check` against the
  published policy before adding automatic fetch/apply behavior.
