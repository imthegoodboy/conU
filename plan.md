...
- Issue #252 tracks this hardening work on branch `manual-update-apply`; the
...
- Open a PR, let CI and branch `Release Artifacts` validate the Rust tests and
  package gates, merge without deleting the branch, then run a real signed
  release dry-run using `conu update check`, `conu update download`, and
  `conu update apply --dry-run` before any confirmed production replacement.
