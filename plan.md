...
- Issue #212 tracks this slice on branch `signed-linux-repository-metadata`.
- Add `scripts/sign-linux-repository-metadata.py` to import the maintainer GPG
...
- Add `scripts/check-linux-repository-signing.py` with an ephemeral GPG-key
...
- Wire CI package checks, Release Artifacts package checks, tagged GitHub
...
- GitHub Actions validation is pending for the Issue #212 PR.

Next:

- Validate locally, open and merge the PR, preserve the branch, update this
  section to completed with PR/check links, then continue with package-manager
  repository submission, hosted repository publication, RPM package payload
  signing, managed public relay hosting, distributed hosted dashboards/adaptive
  ...
  remote/cross-region mailbox retention orchestration, or ICE/STUN/TURN managed
  traversal.
