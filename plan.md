...
- Issue #244 is tracked by PR #245 on branch `release-update-client-check`;
...
- The command validates local policy/sidecar files only; it does not yet fetch
  update metadata over the network or apply updates.
...
- Let PR CI exercise the Rust tests, then run the Release Artifacts workflow on
  this branch. After merge, cut a real signed `v*` release and run
  `conu update check` against the published policy before adding automatic
  fetch/apply behavior.
...
- conU includes an installed local `conu update check` verification command on
  the current hardening branch, but it does not yet fetch update metadata over
  the network or apply updates.
