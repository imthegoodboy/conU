...
Current latest hardening addition: release update apply temp-target allocation
...
`update-apply-temp-target-allocation` is preserved. Validation: `cargo fmt --all -- --check`, `git diff --check`, release-version check, and PR CI passed.
Local Rust validation was attempted but this workstation is missing `link.exe`;
PR CI covered Rust on Ubuntu, macOS, and Windows. Prior completed hardening
