...
- Issue #220 tracks this hardening slice on branch
  `linux-signing-secret-preflight`; the branch must be preserved.
...
- `codex review -c sandbox_mode="danger-full-access" --uncommitted` timed out
  ...
  `Release Artifacts` gates must cover final review.

Next:

- Open PR for Issue #220, verify PR CI and branch `Release Artifacts`, then
  merge without deleting the branch if checks pass.
