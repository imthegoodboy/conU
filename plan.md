...
Current latest hardening addition: downloaded release update metadata write
...
`update-download-metadata-create-new` is preserved. Validation: `cargo fmt --all -- --check`,
`git diff --check`, release-version check, and PR CI passed.
Local Rust validation was attempted but this workstation is missing
`link.exe`/`dlltool.exe`; PR CI covered Rust on Ubuntu, macOS, and Windows.
Prior completed hardening addition: release update backup restore hardening is
