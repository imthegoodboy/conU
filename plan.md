...
- Issue #238 tracks the hardening work on branch
  `github-release-clobber-preflight`; the branch must be preserved.
...
- PR/merge validation is still pending.
...
- Finish local validation, open and merge the PR after CI/release workflow
  checks pass, then update this section with the PR number, merge commit, and
