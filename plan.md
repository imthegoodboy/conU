...
2026-07-02 - Post Phase 15 file payload send release prep completed. Added payload-safe `--file <path>` input for `conu send`, `conu chat`, `conu reply`, and matching `conu messages` forms so agents can send local or remote message bytes without shell-specific stdin piping; bumped Cargo/npm packages to `0.1.12`. Validation passed with GNU fmt/check/clippy/workspace tests, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, and isolated byte-for-byte `send --file` plus `pull` smoke with payload/path leak checks. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: merge through protected `main`, publish `@imthegoodboy/conu@0.1.12`, and run online npm install smoke.
2026-07-02 - Post Phase 15 latest reply and agent next command release prep completed. Added `conu reply <agent-id> --latest (--stdin|--file <path>)` and matching `conu messages reply` support so agents can respond to the newest addressed inbox message without copying a long envelope id, while deriving only reply addressing metadata and keeping payload/path output guarded. Added `conu next <agent-id> [--json]` so an agent can get metadata-only readiness, inbox count, newest envelope metadata, and safe next commands without reading payload bytes. Bumped Cargo/npm packages to `0.1.13`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `.agents/skills/conu-agent-user/SKILL.md`, `.agents/skills/conu-builder/references/agent-gateway-contract.md`, `.agents/skills/conu-builder/references/implementation-guardrails.md`, and `docs/user-install-and-agent-guide.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test --workspace`, focused reply tests, focused next-command tests, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, and isolated byte-for-byte `ready -> send --file -> reply --latest --file -> pull` smoke with payload/path leak checks. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: merge through protected `main`, publish `@imthegoodboy/conu@0.1.13`, and run online npm install smoke.
2026-07-02 - Post Phase 15 agent listen command release prep completed. Added `conu listen <agent-id> [--json]` as the simplest agent-facing blocking inbox check, with IPC processing enabled by default, unregistered-agent setup guidance, metadata-only delivered/timeout output, and safe `pull`/`reply --latest` next commands. Added Listen rows to the Up/Down terminal menu and state-aware connect selector, updated dashboard/setup/smoke/agent guidance to prefer `listen` over the longer `wait --process-ipc --timeout-ms 30000` form, and bumped Cargo/npm packages to `0.1.14`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `.agents/skills/conu-agent-user/SKILL.md`, `.agents/skills/conu-builder/references/agent-gateway-contract.md`, `.agents/skills/conu-builder/references/implementation-guardrails.md`, and `docs/user-install-and-agent-guide.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test -p conu-cli`, GNU `cargo test --workspace`, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, and isolated byte-for-byte `ready -> send --file -> listen -> pull` smoke with payload/path leak checks. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: merge through protected `main`, publish `@imthegoodboy/conu@0.1.14`, and run online npm install smoke.
2026-07-02 - Post Phase 15 portable invite/accept release prep completed. Changed `conu invite`/`conu share` from local pairing aliases into signed public peer-card export helpers, changed `conu accept <invite-card-file|->` into signed card import plus default peer policy grants for messages/streams/rooms, kept `conu pair`/`conu join` as local pairing, updated README and npm README two-PC examples, and bumped Cargo/npm packages to `0.1.17`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `README.md`, `packaging/npm/conu-cli/README.md`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test --workspace`, focused portable invite/accept tests, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: merge through protected `main`, publish `@imthegoodboy/conu@0.1.17`, and run online npm install smoke.
2026-07-02 - Post Phase 15 Render relay URL normalization release prep completed. Accepted copied Render `https://...onrender.com` relay URLs across shared relay endpoint validation, normalizing them to `wss://...` for local config, invite cards, peer trust, sessions, routes, relay delivery, and relay admin/client parsing while continuing to reject plaintext `http://`, credentials, query strings, fragments, traversal, and malformed endpoints without echoing secret-bearing values. Updated CLI help and short README/npm/Render-hosting examples, added Rust regressions for core endpoint normalization, relay parser normalization, CLI `conu online`/`relay setup` storage, and invite export signing, and bumped Cargo/npm packages to `0.1.21`. Changed files: `crates/conu-core/src/relay_endpoint.rs`, `crates/conu-core/src/relay.rs`, `crates/conu-core/src/state.rs`, `crates/conu-core/src/trust.rs`, `crates/conu-core/src/relay_delivery.rs`, `crates/conu-core/src/routes.rs`, `crates/conu-core/src/sessions.rs`, `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `README.md`, `packaging/npm/conu-cli/README.md`, `docs/render-relay-hosting.md`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test --workspace`, focused relay endpoint/parser/CLI tests, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, isolated `conu online https://conu-relay.onrender.com --token-stdin --json` smoke, npm pack dry-run, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: merge through protected `main`, publish `@imthegoodboy/conu@0.1.21`, run online npm install smoke, and deploy the updated main commit to the existing Render `conu-relay` service.
```

2026-07-02 - Post Phase 15 one-command hosted setup release prep completed. Added `conu setup --relay <ws|wss|https endpoint> [--token-stdin] [--verify] [--wait-ms <ms>]` so setup can initialize local agents, configure the hosted relay endpoint, store a relay credential from bounded stdin, and start `conUD` in one flow. The implementation reuses the existing relay endpoint validation, HTTPS-to-WSS normalization, credential storage, and verification path, keeps token/payload/path display guards, updates README/npm/Render-hosting examples, and bumps Cargo/npm packages to `0.1.22`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test --workspace`, full `conu-cli` tests, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, npm pack dry-run, isolated `conu setup --relay https://conu-relay.onrender.com --token-stdin --start --json` smoke, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.

2026-07-02 - Post Phase 15 terminal online setup release prep completed. Replaced the duplicate top-level terminal menu listener row with an `Online` row that restores the terminal cleanly, prompts for a relay URL, accepts an optional no-echo stdin-safe relay token, and runs the existing `conu setup --relay <url> --token-stdin --start` path without putting tokens on argv. Updated the short user/npm/agent setup docs to keep `conu` as the first control room and keep the lower-level `conu relay credential set --stdin` path as an advanced fallback. Bumped Cargo/npm packages to `0.1.23`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `README.md`, `packaging/npm/conu-cli/README.md`, `.agents/skills/conu-agent-user/SKILL.md`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test -p conu-cli`, GNU `cargo test --workspace`, direct `conu menu` and `conu --help` smokes, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, npm pack dry-run, isolated `conu setup --relay https://conu-relay.onrender.com --token-stdin --json` smoke, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.

2026-07-02 - Post Phase 15 connect selector hosted setup release prep completed. Added an `Online setup` action to the interactive `conu connect` selector whenever no remote agents are visible, reusing the same relay URL plus no-echo token prompt as the top-level menu. Updated static `conu connect` next steps to point at `conu setup --relay <relay-url> --token-stdin --start`, fixed the full command reference so `conu help commands` includes setup relay flags, and bumped Cargo/npm packages to `0.1.24`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test -p conu-cli`, GNU `cargo test --workspace`, direct `conu connect`, `conu help commands`, and `conu --version` smokes, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, npm pack dry-run, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.

2026-07-02 - Post Phase 15 invite/sync onboarding release prep completed. Added short top-level `conu sync [--json]` as an alias for session sync, added `Invite peer` and `Sync` actions to the interactive `conu connect` selector when no remote agent is visible, changed static connect/invite/accept guidance to the simpler hosted path `setup -> invite -> accept -> sync -> connect/chat`, and kept legacy `pair`/`join` available as local pairing commands. Updated the README, npm README, and agent-user skill two-PC examples to rely on the configured relay instead of repeating `invite --relay`. Bumped Cargo/npm packages to `0.1.25`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `README.md`, `packaging/npm/conu-cli/README.md`, `.agents/skills/conu-agent-user/SKILL.md`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, GNU `cargo test -p conu-cli`, GNU `cargo test --workspace`, npm CLI package check, TypeScript SDK check, release-version check, npm publish preflight registry check, smoke-output privacy check, deployment asset check, production-readiness `-SmokeOnly`, npm pack dry-run, rebuilt `conu 0.1.25` two-home invite/accept/sync/connect smoke, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.

2026-07-12 - Post Phase 15 online start alias release prep completed. Added `conu online <relay> --token-stdin --start` as the shortest hosted onboarding command, preserving the existing relay-only `conu online <relay>` behavior while routing `--start` through local setup so agents are created and conUD starts after storing the relay endpoint/credential. Updated CLI help, README/npm/Render/agent guidance, added focused CLI coverage for the relay-only and start flows, and bumped Cargo/npm packages to `0.1.26`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, `README.md`, `packaging/npm/conu-cli/README.md`, `docs/render-relay-hosting.md`, `.agents/skills/conu-agent-user/SKILL.md`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, focused `cargo test -p conu-cli online_alias`, GNU `cargo check --workspace --all-targets`, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.

2026-07-12 - Post Phase 15 online start happy-path hardening completed. Fixed the `conu online --start` regression coverage so it proves the successful hosted setup path against an already-live runtime instead of only asserting the missing-`conUD` failure path, tightened the quick-start help to prefer `conu online <relay> --token-stdin --start`, preserved relay-only `conu online` behavior, and bumped Cargo/npm packages to `0.1.27`. Changed files: `crates/conu-cli/src/lib.rs`, Cargo/npm version manifests, `Cargo.lock`, and `plan.md`. Validation passed with GNU `cargo fmt --all -- --check`, full `cargo test -p conu-cli`, GNU `cargo check --workspace --all-targets`, GNU `cargo clippy --workspace --all-targets -- -D warnings`, and `git diff --check`. Known gaps: no platform signing/notarization secrets were added; this simple-launch release remains scoped to existing unsigned launch behavior. Next: publish through protected main, smoke npm, and deploy/verify the existing Render `conu-relay` service.
