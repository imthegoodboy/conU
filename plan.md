...
Latest hardening addition in review: Verified release update artifact download is implemented locally on branch `verified-update-artifact-download`, tracking Issue #248, with branch preservation required after merge.
...
Status: completed locally, in review
...
- Issue #248 tracks this hardening work on branch
  `verified-update-artifact-download`; the branch must be preserved after merge.
...
- Open PR for branch `verified-update-artifact-download`, let PR CI run full
  Rust tests, run branch `Release Artifacts`, merge if green without deleting
  the branch, then cut a real signed `v*` release and exercise both
  `conu update check --policy-url` and `conu update download --policy-url`
  against uploaded assets before designing automatic update apply.
