...
- Issue #246 is tracked on branch `release-update-remote-check`; the branch is
  preserved.
...
- Local `cargo +stable-x86_64-pc-windows-gnu test -p conu-cli update_check -- --nocapture`
  could not link on this Windows workstation because the GNU target lacks
  `dlltool.exe`; CI must run the full Rust test matrix.
...
- Remote mode validates signed update metadata only; automatic update artifact
  download/apply remains future work.

Next recommendation:

- Open PR for Issue #246, let PR CI exercise the linked Rust tests, then run
  the Release Artifacts workflow on this branch. After merge, cut a real signed
  `v*` release and run `conu update check --policy-url` against the published
  policy before adding automatic update artifact download/apply behavior.
...
- Continue with remote policy/sidecar fetch validation, then cut a real signed
  `v*` release and run `conu update check` against the published policy before
  adding automatic update artifact download/apply behavior.
...
- conU includes an installed `conu update check` verification command, and
  remote policy/sidecar fetch support is implemented separately on branch
  `release-update-remote-check`; automatic update artifact download/apply
  remains future work.
...
- Continue with real signing/npm secret configuration, a signed `v*` release,
  ...
  metadata fetching/apply behavior beyond local signed-policy checks, managed
  public relay hosting, distributed hosted dashboards/accounting/adaptive abuse automation,
