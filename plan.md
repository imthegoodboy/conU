...
- Issue #204 tracks this slice on branch `rpm-release-assets`; the branch must
  be preserved.
...
- Local Windows and WSL validation did not exercise native RPM generation
  ...
  branch `Release Artifacts` workflow must exercise the RPM path on Ubuntu.

Next:

- Open a PR for branch `rpm-release-assets`, verify PR CI plus branch
  `Release Artifacts`, merge without deleting the branch, then update this plan
  with the PR number, merge commit, CI links, and final status.
