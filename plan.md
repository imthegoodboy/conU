...
Current latest hardening addition: release update apply backup preflight
...
`update-apply-backup-preflight` is preserved. Validation: `cargo fmt --all -- --check`, `git diff --check`, release-version check, and PR CI passed.
...
Prior completed hardening addition: release update apply temp-target allocation
