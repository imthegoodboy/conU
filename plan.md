...
- Issue #254 tracks this hardening work on branch
  `published-update-apply-gate`; PR/merge closeout is still pending.
...
- Open the PR, require CI plus branch `Release Artifacts`, merge without
  deleting `published-update-apply-gate`, then observe the next signed `v*`
  release to prove the public check/download/apply dry-run gate against real
  uploaded assets before npm publication.
