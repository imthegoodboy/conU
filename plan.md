...
- Issue #214 tracks this slice on branch `linux-gpg-public-key-release-asset`.
- Add `scripts/export-linux-gpg-public-key.py` to import the existing Linux GPG
  signing private key into a temporary `GNUPGHOME`, export only
  `conu-linux-gpg-key.asc`, and write a strict `.sha256` sidecar.
- Add `scripts/check-linux-gpg-public-key-export.py` to use an ephemeral GPG
...
- Wire CI, Release Artifacts package checks, tagged GitHub Release publication,
...
- PR CI and branch `Release Artifacts` checks are still pending.

Next:

- Finish validation, open and merge the PR without deleting the branch, then
  update this section with completion details and remaining gaps.
