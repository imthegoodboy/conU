...
Direct is selected only after a live QUIC connection succeeds and the peer answers an encrypted challenge with the trusted peer-card key. If direct is unavailable, conU records `direct_quic_probe_failed` and keeps relay selected. Relay-backed message, stream-chunk, and room-event delivery remains available for trusted peers.
