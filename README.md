...
Release packaging/readiness checks include bounded archive verification, release artifact smoke preflights for missing or non-file binaries, package-manager manifest and APT/RPM metadata generation regressions, verified package-manager submission bundle regressions, hosted Linux repository site cache-policy regressions, custom S3-compatible hosted repository publication regressions, hosted Linux repository endpoint Cache-Control readiness regressions, signed release update-policy regressions, installed `conu update check` local and bounded HTTPS policy-sidecar validation, installed `conu update download` selected archive checksum/signature verification, consent-gated `conu update apply` archive validation/install dry-runs, a tagged-release public update check/download/apply dry-run gate after GitHub Release upload, Linux signing-secret preflight regressions, Windows/macOS signing-secret value preflight regressions, live tagged-release readiness regressions, GitHub main branch protection readiness regressions, GitHub Actions permissions readiness regressions, GitHub workflow permissions readiness regressions, GitHub repository security readiness regressions, native RPM package-signing regressions, fingerprint-pinned Linux detached/repository signing regressions, Linux GPG public-key export regressions, npm launcher local-binary preflights, deterministic npm package-content verification, artifact attestations, and tagged release signing/npm secret gates.
...
The release artifact includes `conu`, `conud`, `conu-relay`, `conu-mcp`, docs, packaging templates, checksum files, and a manifest that states `payload_contents_included = false`. Service templates live under `packaging/` for Windows, Linux systemd, macOS launchd, Docker relay hosting, npm launcher package, and generated package-manager manifest guidance. Tagged release builds run a fail-closed preflight for Windows Authenticode signing, macOS Developer ID signing/notarization, Windows/macOS PKCS#12 certificate value parsing, fingerprint-pinned Linux GPG signing secret presence, key import, full-fingerprint match, and probe signing, `NPM_TOKEN` publication, npm package publish metadata, package/tag version consistency, package-manager manifest generation, GitHub Release clobber safety, target commit CI status, default release branch head match, GitHub Release asset publication metadata, and existing npm package-version conflicts before platform builds or npm publication. Before uploading release secrets, maintainers can run `python scripts/set-github-release-secrets.py --write-env-template .env.release` to generate the ignored local template, run `python scripts/set-github-release-secrets.py --env-file .env.release --check-env-file` after filling it to verify every required name has a non-empty value without GitHub access, then run `python scripts/set-github-release-secrets.py --repo <owner/name> --env-file .env.release --env-file-only --dry-run --preflight-values --require-openssl` or export values directly to validate Windows/macOS and Linux signing values, then rerun without `--dry-run` so GitHub CLI receives values through stdin. Before creating the tag, maintainers can run `python scripts/check-tagged-release-readiness.py --repo <owner/name> --tag v<version> --npm-registry-check --require-ci --require-default-branch-head` to combine live secret-name checks, default Pages or custom S3 repository settings, GitHub Release clobber status, package version/tag consistency, target commit CI status, default release branch head status, and npm target availability without printing secret values, CI logs, or variable values. Release artifacts get GitHub attestations for each archive and checksum file, Linux archives use SHA-256 plus GitHub attestations plus detached `.asc` signatures, generated Debian packages use SHA-256 sidecars plus detached `.asc` signatures, generated RPM packages use native RPM signatures plus refreshed SHA-256 sidecars plus detached `.asc` signatures, generated APT/RPM metadata ZIPs add native repository signatures before receiving detached `.asc` signatures, and `conu-linux-gpg-key.asc` lets users verify Linux signatures from release assets after comparing its full fingerprint with the published maintainer fingerprint. The release workflow verifies archives, generates Homebrew, Scoop, winget, Chocolatey, Debian, and RPM package-manager files plus RPM packages from strict release checksums, verifies the imported Linux signing key matches `CONU_LINUX_GPG_KEY_FINGERPRINT`, signs generated RPM packages before generating RPM repository metadata, exports the Linux GPG public key, adds APT `InRelease`/`Release.gpg` and RPM `repodata/repomd.xml.asc` signatures, signs Linux release assets after package generation, prepares and signs `conu-<version>-package-manager-submissions.zip` with Homebrew tap, Scoop bucket, winget-pkgs, Chocolatey, Debian, APT, RPM, and Linux signing files in repository-ready paths, generates `conu-<version>-update-policy.json` with public release URLs, strict hashes, signature URLs, npm package versions, and auto-apply disabled, signs that update policy, lets installed clients validate it from local files or a bounded public HTTPS policy URL with `conu update check`, lets operators fetch one selected platform archive plus sidecars with `conu update download`, lets operators dry-run or explicitly confirm installation from that verified archive with `conu update apply`, and after GitHub Release upload imports the published Linux public key, verifies its fingerprint, then runs `conu update check --policy-url --gpg-verify`, `conu update download --policy-url --gpg-verify`, and `conu update apply --policy-url --gpg-verify --dry-run` against the public release before npm publication, smoke-tests unpacked installs, smoke-tests the npm launcher local install path with an existing regular-file binary directory, runs local-smoke, package-manager, package-manager submission, release-update-policy, release-update-download/apply-gate, Linux-signing-secret-preflight, platform-signing-secret-value-preflight, tagged-release-readiness, RPM-package-signing, Linux-signing, Linux-repository-signing, Linux-public-key-export, GitHub Release clobber, GitHub Release asset publication, and npm-publish preflight regression checks, and smoke-tests the npm launcher download/checksum install path with HTTPS-or-loopback URL enforcement, bounded timeout/size behavior, archive-member count/duplicate/state-path preflight, and bounded extracted-tree binary selection before attestation/upload. Before GitHub Release asset upload, tagged publication verifies no public release already exists for the tag and refuses to overwrite assets; before npm registry access, tagged npm publication verifies the public GitHub Release has all expected platform archives, checksum sidecars, Linux signatures, package-manager files, signed package-manager submission bundle, public-key asset, hosted repository bundle, hosted repository site artifact with checked cache policy files, and signed update policy metadata. See `docs/release-checklist.md`, `docs/platform-code-signing.md`, `docs/observability.md`, `docs/distribution-and-hosting.md`, and `packaging/README.md`.
...
cargo run -p conu-cli -- update check --policy-file dist/conu-0.1.0-update-policy.json --gpg-verify --json
cargo run -p conu-cli -- update check --policy-url https://github.com/imthegoodboy/conU/releases/download/v0.1.0/conu-0.1.0-update-policy.json --gpg-verify --json
cargo run -p conu-cli -- update download --policy-url https://github.com/imthegoodboy/conU/releases/download/v0.1.0/conu-0.1.0-update-policy.json --output-dir dist/update-downloads --target linux-x64 --gpg-verify --json
cargo run -p conu-cli -- update apply --policy-url https://github.com/imthegoodboy/conU/releases/download/v0.1.0/conu-0.1.0-update-policy.json --artifact-file dist/update-downloads/conu-0.1.0-linux-x64.tar.gz --install-dir /usr/local/bin --target linux-x64 --gpg-verify --dry-run --json
