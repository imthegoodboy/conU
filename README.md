...
Managed operators can also run `conu-relay --hosted-fleet-dashboard --fleet-file <path> [--account <id>] [--node <id>] [--json]` to aggregate a guarded manifest of multiple relay-local metadata stores without printing tokens, token hashes, session ids, payloads, ciphertext bodies, or frame contents. This is a fleet snapshot, not hosted billing or adaptive abuse automation.
