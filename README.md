...
Guarded fleet account suspension uses the same fleet manifest as hosted fleet dashboards. Run `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> --dry-run [--json]`, review the aggregate and per-relay credential/tenant counts, then rerun with `--confirm` to revoke tenant metadata first and account credential records second on every complete local source. Each affected relay entry must include both `credentials_file` and `tenants_file`; partial credential/tenant entries fail closed, all complete sources are preflighted before confirmed mutation, output is metadata-only, and the command never contacts remote relays.
