...
Guarded local fleet account audit and suspension are the current tenant-workflow bridge beyond one relay at a time: `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file <path> [--node <node-id>] [--fail-on-warning]` reuses the versioned fleet manifest to report credential/tenant source coverage and account or account/node consistency warning categories without mutation, and `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> (--dry-run|--confirm)` requires complete `credentials_file` plus `tenants_file` entries for affected relays, validates all complete local sources before confirmed mutation, revokes tenant metadata before credentials, and reports only aggregate/per-relay metadata and display guards. Neither command contacts remote relays, coordinates distributed locks, or provides a managed tenant workflow service.
