...
Guarded local fleet account suspension is the current tenant-workflow bridge beyond one relay at a time: `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> (--dry-run|--confirm)` reuses the versioned fleet manifest, requires complete `credentials_file` plus `tenants_file` entries for affected relays, validates all complete local sources before confirmed mutation, revokes tenant metadata before credentials, and reports only aggregate/per-relay metadata and display guards. It does not contact remote relays, coordinate distributed locks, or provide a managed tenant workflow service.
