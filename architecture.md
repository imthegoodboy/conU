...
The current relay supports offline scoped credential issuance, helper-driven manifest upsert/rotation/revocation, account-scoped online issue/rotate/revoke/audit, local and admin-gated single-relay account suspension, guarded local fleet account or account-node audit/suspension, guarded local fleet account/node credential revoke, guarded local fleet tenant-node upsert/revoke, admin-gated online tenant account/node lifecycle, manifest-backed scoped admin tokens for credentials/tenants/dashboard/session/mailbox actions with payload-safe local manifest audits, and a live-reloaded hashed credential manifest for token revocation/expiry on new session authentication attempts. That is enough for a single managed relay operator to control node credentials, hosted tenant metadata, and payload-safe session-state visibility without storing raw node tokens server-side or handing every operator a full-admin secret; it is not distributed tenant lifecycle, tenant-wide workflow automation across relays, or a full managed public relay service.
...
Guarded local fleet account audit, credential revoke, tenant-node lifecycle, and suspension are the current tenant-workflow bridge beyond one relay at a time: `conu-relay --hosted-fleet-account-audit <account-id> --fleet-file <path> [--node <node-id>] [--fail-on-warning]` reuses the versioned fleet manifest to report credential/tenant source coverage and account or account/node consistency warning categories without mutation; `conu-relay --hosted-fleet-credential-revoke <account-id> <node-id> --fleet-file <path> (--dry-run|--confirm)` preflights every configured local `credentials_file`, requires exactly one target account/node credential per source, and rejects ownership collisions or duplicate node records before mutating account/node credential metadata; `conu-relay --hosted-fleet-tenant-node-upsert <account-id> <node-id> --fleet-file <path> (--dry-run|--confirm)` and `conu-relay --hosted-fleet-tenant-node-revoke <account-id> <node-id> --fleet-file <path> (--dry-run|--confirm)` preflight every configured local `tenants_file` before mutating tenant-node metadata; and `conu-relay --hosted-fleet-account-suspend <account-id> --fleet-file <path> [--node <node-id>] (--dry-run|--confirm)` requires complete `credentials_file` plus `tenants_file` entries for affected relays, validates all complete local sources before confirmed mutation, revokes tenant metadata before credentials for account-wide mode, revokes tenant-node metadata before node credentials for node mode, and reports only aggregate/per-relay metadata and display guards. None of these commands contacts remote relays, coordinates distributed locks, or provides a managed tenant workflow service.
