...
- The relay can persist metadata-only abuse/dashboard counter files under `CONU_RELAY_ABUSE_DIR` and render them with `conu-relay --abuse-audit --abuse-dir <path>`. It can also render a local hosted dashboard snapshot with `conu-relay --hosted-dashboard --credentials-file <path> --tenants-file <path> --accounting-dir <path> --abuse-dir <path>` or an admin-gated online snapshot with `conu-relay --admin-hosted-dashboard --relay <wss://...> --admin-token-stdin`. Managed operators can update and audit one configured tenant registry online with `conu-relay --admin-tenant-upsert`, `--admin-tenant-node-upsert`, `--admin-tenant-node-revoke`, `--admin-tenant-revoke`, and `--admin-tenant-audit`, always with `--admin-token-stdin`. `CONU_RELAY_ADMIN_TOKENS_FILE` can live-read hashed admin tokens scoped to credentials, tenants, dashboard, mailbox audit, and mailbox purge actions. Abuse/dashboard/tenant/mailbox audit and purge output contains aggregate enforcement, tenant, or retention counters, optional node ids, window/timestamp metadata, and display guards only, not tokens, token hashes, admin tokens, session ids, payloads, ciphertext bodies, private keys, or frame contents.
...
- Distributed multi-instance session migration/accounting, distributed dashboards, adaptive abuse response, hosted tenant lifecycle/workflow automation beyond one relay's scoped admin tenant commands, and distributed hosted mailbox retention orchestration remain future hardening work.
