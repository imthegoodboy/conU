...
archive hashing, archive-member preflight,
...
It downloads the native release archive from GitHub Releases with bounded request time and response sizes, requires a strict checksum line naming that archive, hashes the archive in chunks, and exposes `conu`, `conud`, `conu-relay`, and `conu-mcp`.
