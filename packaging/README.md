...
They also run `python scripts/check-hosted-linux-repository-site.py`, `python scripts/check-hosted-linux-repository-pages.py`, `python scripts/check-hosted-linux-repository-endpoint-regression.py`, `python scripts/check-hosted-linux-repository-s3-publication.py`, and `python scripts/check-github-pages-readiness-regression.py` so static hosted repository site, GitHub Pages extraction, custom S3 publication, live endpoint cache-header, and GitHub Pages metadata readiness regressions fail before tagged package publication paths are used.
...
package/metadata assets with detached `.asc` signatures, prepares and signs
`conu-<version>-package-manager-submissions.zip` with repository-ready
Homebrew/Scoop/winget/Chocolatey/Debian/APT/RPM paths, and uploads
...
published Linux public key, verifies its fingerprint, and runs public
`conu update check --policy-url --gpg-verify`, public `conu update download
--policy-url --target linux-x64 --gpg-verify`, and public `conu update apply
--policy-url --target linux-x64 --gpg-verify --dry-run` before npm publication.
...
upload. The S3 publication regression validates that every extracted site file
gets exactly one generated cache rule and that publication passes per-object
cache metadata to the AWS CLI without exposing credentials. The S3 publication
script can publish a verified extracted site to the supported S3-compatible
static host path, and the endpoint readiness check can be run after publication
to prove `repository.json`, `cache-policy.json`, `_headers`, and live
`Cache-Control` headers match the generated cache policy. DNS records,
certificates, CDN invalidation, non-S3 custom hosts, package-manager submission,
and unattended automatic update apply behavior remain future work.
