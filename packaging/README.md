...
They also run `python scripts/check-hosted-linux-repository-site.py` and `python scripts/check-hosted-linux-repository-pages.py` so static hosted repository site and GitHub Pages extraction regressions fail before tagged package publication paths are used.
...
HTTPS base URL requirement, install snippets, `repository.json`, deterministic
ZIP metadata, download copies, and fail-closed handling for missing signatures
or unsafe bundle members. The Pages regression validates strict site ZIP
sidecars, detached signature presence, safe extraction to an empty deployment
directory, non-HTTPS metadata rejection, and forbidden local-state/secret marker
rejection before GitHub Pages upload. Custom DNS/TLS/cache policy and
