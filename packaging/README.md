...
They also run `python scripts/check-release-update-download-gate.py` so tagged GitHub Release publication cannot lose the post-upload public update check/download GPG verification gate before npm publication.
...
--policy-url --target linux-x64 --gpg-verify` before npm publication.
